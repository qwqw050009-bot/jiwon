...
돌려줄 수 있다. Cloudflare Pages는 루트의 `dist/404.html`이 있으면
없는 URL을 진짜 404로 돌린다. `/* /404.html 404` 같은 `_redirects`
스플랫은 쓰지 않는다 — 정적 파일이 있어도 리다이렉트가 적용되고,
404 상태 재작성은 지원하지 않아 `/`·`/urgent/` 같은 지원 URL을
깨뜨릴 수 있다. 배포 후 미생성 조합 URL의 실제 404 상태코드는
한 번 더 확인할 것.
